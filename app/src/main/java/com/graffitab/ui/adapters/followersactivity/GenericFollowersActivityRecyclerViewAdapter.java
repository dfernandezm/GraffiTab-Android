...
public class GenericFollowersActivityRecyclerViewAdapter extends AdvancedEndlessRecyclerViewAdapter<GTActivityContainer> {
    ...
    private final int VIEW_TYPE_CREATE_STREAMABLE = 3;
    ...
    public int getViewType(int position) {
        GTActivity.GTActivityType type = getItem(position).type;
        switch (type) {
    ...
    public RecyclerView.ViewHolder onCreateItemViewHolder(ViewGroup parent, int viewType) {
        switch (viewType) {
            case VIEW_TYPE_LIKE: {
                View layoutView = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_activity_like, parent, false);
